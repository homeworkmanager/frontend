...
import { usePatchNoteMutation } from '@/utils/store/middleware/endpoints/note';
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        <Button
          variant="accept"
          disabled={patchUpdateNoteState.isLoading || !noteText || noteText === note.note_text}
          onClick={updateNote}
        >
          {patchUpdateNoteState.isLoading ? <Loader /> : 'Изменить'}
        </Button>
        {patchUpdateNoteState.isError && <Toast type="mobile" text="Ошибка изменения заметки" />}
      </motion.aside>
    </AnimatePresence>
  );
};
